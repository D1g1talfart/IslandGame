...
# ============================================================================
# EXPORT VARIABLES - Organized by Generation Phase
# ============================================================================
...
var terrain_data: Array[Array] = []
var height_data: Array[Array] = []
var water_types: Array[Array] = []
var beach_zones: Array[Array] = []
...
func _ready():
	add_to_group("island_generator")
	if not tile_set:
		print("Warning: No TileSet resource assigned to TileMapLayer")
		return
	
	setup_noise()
	
	# Only auto-generate if enabled (should be false by default)
	if auto_generate_on_ready:
		generate_advanced_island()
	else:
		print("Island generator ready - waiting for manual trigger")
...
func initialize_data_arrays():
	...
	for y in range(island_height):
		...
		for x in range(island_width):
			terrain_data[y].append(TerrainType.DEEP_OCEAN)
			height_data[y].append(0.0)
			water_types[y].append(WaterType.NONE)
			beach_zones[y].append(false)
...
func generate_new_island():
	"""Call this from your menu button to start generation"""
	if is_generating:
...
func generate_advanced_island():
	if is_generating:
		print("Generation already in progress, skipping...")
		return
	
	is_generating = true
	generation_started.emit()  # ADD THIS
	
	setup_noise()
	clear()
	initialize_data_arrays()
	
	generation_progress.emit("Generating elevation...", 0.1)  # REPLACE print()
	generate_elevation_with_gradient()
	
	generation_progress.emit("Smoothing terrain...", 0.2)  # REPLACE print()
	smooth_terrain_post_generation() 
	
	generation_progress.emit("Setting initial ocean...", 0.3)  # REPLACE print()
	set_initial_ocean_tiles() 
	
	generation_progress.emit("Placing terrain tiles...", 0.4)  # REPLACE print()
	place_all_terrain_tiles()
	
	generation_progress.emit("Placing deep ponds...", 0.5)  # REPLACE print()
	place_deep_ponds()
	
	generation_progress.emit("Generating rivers...", 0.6)  # REPLACE print()
	generate_rivers_from_ponds()
	
	generation_progress.emit("Creating river mouths...", 0.7)  # REPLACE print()
	create_river_mouths() 
	
	generation_progress.emit("Creating beaches...", 0.8)  # REPLACE print()
	create_beaches()
	
	generation_progress.emit("Adding shallow water zones...", 0.9)  # REPLACE print()
	create_shallow_water_zones()
	
	generation_progress.emit("Adding small ponds...", 0.95)  # REPLACE print()
	add_small_freshwater_ponds()
	
	generation_progress.emit("Finalizing island...", 1.0)  # REPLACE print()
	center_camera_on_island()
	is_generating = false
	
	generation_complete.emit()  # ADD THIS

func regenerate():
	if is_generating:
		print("Generation in progress, cannot regenerate now")
		return
	generate_advanced_island()
		...
		for neighbor in neighbors:
			if is_valid_position(neighbor):
				...
				if neighbor_key not in ocean_connected:
					var neighbor_height = height_data[neighbor.y][neighbor.x]
					
					if neighbor_height <= water_threshold or terrain_data[neighbor.y][neighbor.x] == TerrainType.DEEP_OCEAN:
						ocean_connected[neighbor_key] = true
						to_check.append(neighbor)
	
	return ocean_connected

func find_interior_water_patches_improved(ocean_connected: Dictionary) -> Array[Array]:
	var interior_patches: Array[Array] = []
	var visited = {}
...
func flood_fill_interior_water_patch(start_pos: Vector2i, visited: Dictionary, ocean_connected: Dictionary) -> Array[Vector2i]:
	...
	while to_check.size() > 0:
		...
		var height = height_data[current_pos.y][current_pos.x]
...
func fill_water_patch_with_terrain(patch: Array[Vector2i]):
	...
	for pos in patch:
		height_data[pos.y][pos.x] = fill_height + randf_range(-0.02, 0.02)
...
func find_small_elevation_patches() -> Array[Dictionary]:
	...
	for y in range(1, island_height - 1):
		for x in range(1, island_width - 1):
			...
			var current_height = height_data[y][x]
...
func smooth_elevation_patch_to_level(patch: Array[Vector2i], target_level: int):
	...
	for pos in patch:
		...
		height_data[pos.y][pos.x] = target_height + height_variation
...
func smooth_cliff_transitions():
	...
	for adjustment in tiles_to_adjust:
		...
		height_data[pos.y][pos.x] = new_height + height_variation
...
func can_place_deep_pond_at_level(center_x: int, center_y: int, target_level: int) -> bool:
	...
	for existing_pond in deep_ponds:
...
func create_deep_pond_at_terrain_level(center_x: int, center_y: int):
	var base_height = height_data[center_y][center_x]
	...
	for pos in pond_shape:
		if is_valid_position(pos):
			var tile_level = get_elevation_level(height_data[pos.y][pos.x])
			if tile_level == terrain_level:
				terrain_data[pos.y][pos.x] = TerrainType.DEEP_FRESHWATER_POND
				water_types[pos.y][pos.x] = WaterType.FRESH
				height_data[pos.y][pos.x] = pond_height
				set_cell(pos, deep_freshwater_id, Vector2i(0, 0))
...
func create_shallow_pond_at_terrain_level(center_x: int, center_y: int):
	...
	for pos in pond_shape:
		if is_valid_position(pos):
			var tile_level = get_elevation_level(height_data[pos.y][pos.x])
			var current_terrain = terrain_data[pos.y][pos.x]
			...
			if tile_level == terrain_level and current_terrain in [
				...
				terrain_data[pos.y][pos.x] = TerrainType.SHALLOW_FRESHWATER
				water_types[pos.y][pos.x] = WaterType.FRESH
				height_data[pos.y][pos.x] = pond_height
				set_cell(pos, shallow_freshwater_id, Vector2i(0, 0))
...
func generate_rivers_from_ponds():
	...
	print("Found ", deep_ponds.size(), " deep ponds")
	
	var rivers_created = 0
	
	for i in range(deep_ponds.size()):
		var pond_pos = deep_ponds[i]
		var pond_level = get_elevation_level(height_data[pond_pos.y][pond_pos.x])
	...
	print("Created ", rivers_created, " rivers from ", deep_ponds.size(), " ponds")
...
func generate_simple_river(pond_pos: Vector2i) -> bool:
	...
	var current_level = get_elevation_level(height_data[current_pos.y][current_pos.x])
	...
	for step in range(200):
		...
		var next_level = get_elevation_level(height_data[next_pos.y][next_pos.x])
...
func find_connected_pond_tiles(start_pos: Vector2i) -> Array[Vector2i]:
	...
	for search_pos in search_positions:
		if is_valid_position(search_pos):
			if terrain_data[search_pos.y][search_pos.x] == TerrainType.DEEP_FRESHWATER_POND:
	...
	while to_check.size() > 0:
		...
		if terrain_data[current_pos.y][current_pos.x] != TerrainType.DEEP_FRESHWATER_POND:
			continue
...
func find_pond_perimeter_positions(pond_tiles: Array[Vector2i]) -> Array[Vector2i]:
	...
	for pond_tile in pond_tiles:
		...
		for direction in directions:
			...
			var terrain = terrain_data[adjacent_pos.y][adjacent_pos.x]
...
func is_connected_to_substantial_landmass(start_pos: Vector2i, min_landmass_size: int = 20) -> bool:
	...
	while to_check.size() > 0 and landmass_size < max_check:
		...
		var terrain = terrain_data[current_pos.y][current_pos.x]
...
func has_downhill_flow_potential(pos: Vector2i) -> bool:
	var start_height = height_data[pos.y][pos.x]
	...
	for radius in range(1, 6):
		for dy in range(-radius, radius + 1):
			for dx in range(-radius, radius + 1):
				...
				var check_height = height_data[check_pos.y][check_pos.x]
...
func is_northern_flow_risk(pos: Vector2i, pond_center: Vector2i) -> bool:
	...
	for check_y in range(pos.y - 1, -1, -1):
		...
		var terrain = terrain_data[check_pos.y][check_pos.x]
...
func has_clear_path_potential(pos: Vector2i) -> bool:
	var current_height = height_data[pos.y][pos.x]
...
func find_next_river_step(pos: Vector2i, max_level: int, visited: Array[Vector2i]) -> Vector2i:
	...
	for direction in get_flow_directions():
		...
		var terrain = terrain_data[check_pos.y][check_pos.x]
		var terrain_level = get_elevation_level(height_data[check_pos.y][check_pos.x])
...
func calculate_simple_score(from_pos: Vector2i, to_pos: Vector2i, direction: Vector2i) -> float:
	...
	var height_diff = height_data[from_pos.y][from_pos.x] - height_data[to_pos.y][to_pos.x]
...
func is_valid_river_endpoint(pos: Vector2i) -> bool:
	var terrain = terrain_data[pos.y][pos.x]
...
func widen_river_path(river_path: Array[Vector2i]):
	...
	for i in range(river_path.size()):
		...
		if current_terrain in [TerrainType.RIVER, TerrainType.RIVER_1, TerrainType.RIVER_2, TerrainType.RIVER_3, TerrainType.RIVER_MOUTH]:
			...
			river_levels.append(get_elevation_level(height_data[pos.y][pos.x]))
			continue
...
func place_river_tile_with_level(pos: Vector2i) -> int:
	...
	if current_terrain in [TerrainType.RIVER, TerrainType.RIVER_1, TerrainType.RIVER_2, TerrainType.RIVER_3, TerrainType.RIVER_MOUTH, TerrainType.DEEP_OCEAN, TerrainType.SHALLOW_SALTWATER]:
		return get_elevation_level(height_data[pos.y][pos.x])
	...
	var river_tile_id: int
	
	match terrain_level:
		1:
			river_terrain = TerrainType.RIVER_1
			river_tile_id = river_1_id
		2:
			river_terrain = TerrainType.RIVER_2
			river_tile_id = river_2_id
		3:
			river_terrain = TerrainType.RIVER_3
			river_tile_id = river_3_id
		_:
			river_terrain = TerrainType.RIVER
			river_tile_id = river_id
	
	terrain_data[pos.y][pos.x] = river_terrain
	water_types[pos.y][pos.x] = WaterType.FRESH
	
	var river_height = calculate_river_height_for_level(terrain_level)
	height_data[pos.y][pos.x] = river_height
	
	set_cell(pos, river_tile_id, Vector2i(0, 0))
...
func add_river_width_at_level(center_pos: Vector2i, flow_direction: Vector2i, width: int, required_level: int):
	...
	for direction in perpendicular_dirs:
		for distance in range(1, tiles_per_side + 1):
			...
			var terrain = terrain_data[side_pos.y][side_pos.x]
			...
			var side_terrain_level = get_elevation_level(height_data[side_pos.y][side_pos.x])
...
func get_average_surrounding_terrain_height(pos: Vector2i) -> float:
	...
	for dy in range(-1, 2):
		for dx in range(-1, 2):
			...
			if is_valid_position(check_pos):
				var terrain = terrain_data[check_pos.y][check_pos.x]
				var height = height_data[check_pos.y][check_pos.x]
	...
	return total_height / float(count) if count > 0 else height_data[pos.y][pos.x]
...
func create_river_mouths():
	...
	for y in range(island_height):
		for x in range(island_width):
			var current_terrain = terrain_data[y][x]
...
func river_touches_ocean(river_x: int, river_y: int) -> bool:
	...
	for direction in directions:
		...
		var neighbor_water_type = water_types[check_pos.y][check_pos.x]
...
func create_mouth_at_river_end(river_pos: Vector2i, processed_positions: Dictionary):
	...
	for tile_pos in mouth_tiles:
		if is_valid_position(tile_pos):
			var current_terrain = terrain_data[tile_pos.y][tile_pos.x]
			
			if should_become_river_mouth(current_terrain):
				terrain_data[tile_pos.y][tile_pos.x] = TerrainType.RIVER_MOUTH
				water_types[tile_pos.y][tile_pos.x] = WaterType.FRESH
				set_cell(tile_pos, river_mouth_id, Vector2i(0, 0))
				
				var pos_key = str(tile_pos.x) + "," + str(tile_pos.y)
				processed_positions[pos_key] = true
				
				if tile_pos not in river_mouths:
					river_mouths.append(tile_pos)
	...
	for y in range(island_height):
		for x in range(island_width):
			...
			if current_terrain in [
				...
				terrain_data[y][x] = TerrainType.BEACH
				height_data[y][x] = beach_threshold
				beach_zones[y][x] = true
				
				set_cell(Vector2i(x, y), beach_id, Vector2i(0, 0))
				beaches_placed += 1
...
func is_northern_shore(x: int, y: int) -> bool:
	...
	for i in range(1, 4):
		if y + i < island_height:
			var south_terrain = terrain_data[y + i][x]
	...
	for i in range(1, 4):
		if y - i >= 0:
			var north_terrain = terrain_data[y - i][x]
...
func add_beach_extensions():
	...
	for y in range(island_height):
		for x in range(island_width):
			if terrain_data[y][x] == TerrainType.BEACH:
...
func extend_beach_into_ocean(beach_x: int, beach_y: int):
	...
	for i in range(1, south_extension + 1):
		...
		if is_valid_position(extend_pos) and terrain_data[extend_pos.y][extend_pos.x] == TerrainType.DEEP_OCEAN:
			terrain_data[extend_pos.y][extend_pos.x] = TerrainType.BEACH
			beach_zones[extend_pos.y][extend_pos.x] = true
			set_cell(extend_pos, beach_id, Vector2i(0, 0))
...
func create_shallow_water_zones():
	...
	for river_mouth in river_mouths:
...
func add_shallow_saltwater_around_beach(beach_x: int, beach_y: int):
	...
	for depth in range(1, shallow_saltwater_depth + 1):
		for direction in directions:
			...
			if is_valid_position(check_pos):
				var current_terrain = terrain_data[check_pos.y][check_pos.x]
				if current_terrain == TerrainType.DEEP_OCEAN:
					terrain_data[check_pos.y][check_pos.x] = TerrainType.SHALLOW_SALTWATER
					water_types[check_pos.y][check_pos.x] = WaterType.SALT
					height_data[check_pos.y][check_pos.x] = water_threshold
					set_cell(check_pos, shallow_saltwater_id, Vector2i(0, 0))
...
func add_shallow_freshwater_around_river_mouth(mouth_pos: Vector2i):
	...
	for depth in range(1, 3):
		for direction in directions:
			...
			var current_terrain = terrain_data[check_pos.y][check_pos.x]
			var current_water_type = water_types[check_pos.y][check_pos.x]
			
			if current_terrain == TerrainType.DEEP_OCEAN and current_water_type == WaterType.SALT:
				terrain_data[check_pos.y][check_pos.x] = TerrainType.SHALLOW_FRESHWATER
				water_types[check_pos.y][check_pos.x] = WaterType.FRESH
				set_cell(check_pos, shallow_freshwater_id, Vector2i(0, 0))
				...
				terrain_data[check_pos.y][check_pos.x] = TerrainType.BEACH
				set_cell(check_pos, shallow_freshwater_id, Vector2i(0, 0))
...
func set_initial_ocean_tiles():
	for y in range(island_height):
		for x in range(island_width):
			var height = height_data[y][x]
			if height <= water_threshold:
				terrain_data[y][x] = TerrainType.DEEP_OCEAN
				water_types[y][x] = WaterType.SALT

func place_all_terrain_tiles():
	for y in range(island_height):
		for x in range(island_width):
			var height = height_data[y][x]
			var current_terrain = terrain_data[y][x]
			...
			if current_terrain == TerrainType.DEEP_OCEAN and height > water_threshold:
				...
				terrain_data[y][x] = new_terrain

			var tile_id = get_tile_id_for_terrain_type(terrain_data[y][x])
...
func get_tile_id_for_terrain_type(terrain_type: TerrainType) -> int:
	match terrain_type:
...
func center_camera_on_island():
	var camera = get_node_or_null("Camera2D")
	if not camera:
		camera = get_parent().get_node_or_null("Camera2D")
	
	if camera:
		var center_x = island_width * 16
		var center_y = island_height * 16
...
# ============================================================================
# GAMEPLAY INTERFACE FUNCTIONS
# ============================================================================

func get_terrain_at(x: int, y: int) -> TerrainType:
	if not is_valid_position(Vector2i(x, y)):
		return TerrainType.DEEP_OCEAN
	return terrain_data[y][x]

func get_water_type_at(x: int, y: int) -> WaterType:
	if not is_valid_position(Vector2i(x, y)):
		return WaterType.SALT
	return water_types[y][x]
...
func is_wadeable(x: int, y: int) -> bool:
	var terrain = get_terrain_at(x, y)
	return terrain == TerrainType.SHALLOW_SALTWATER or terrain == TerrainType.SHALLOW_FRESHWATER

func is_swimmable(x: int, y: int) -> bool:
	var terrain = get_terrain_at(x, y)
	return terrain == TerrainType.DEEP_OCEAN or terrain == TerrainType.DEEP_FRESHWATER_POND

func can_grow_plants(x: int, y: int) -> bool:
	var terrain = get_terrain_at(x, y)
	return terrain in [
		TerrainType.LEVEL0_GRASS,
		TerrainType.LEVEL1_GRASS, 
...
func can_place_building(x: int, y: int, building_size: Vector2i) -> bool:
	for dy in range(building_size.y):
		for dx in range(building_size.x):
			var check_x = x + dx
			var check_y = y + dy
			
			if not is_walkable(check_x, check_y):
				return false
			
			var base_level = get_elevation_level(get_height_at(x, y))
			var check_level = get_elevation_level(get_height_at(check_x, check_y))
			if check_level != base_level:
				return false
